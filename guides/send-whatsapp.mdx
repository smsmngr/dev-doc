---
title: "Send WhatsApp Messages via SmsManager"
sidebarTitle: "Send WhatsApp"
description: "Guide to sending WhatsApp template and text messages using SmsManager, including template setup and sender configuration."
---
...
<Steps>
  ...
  <Step title="Send the template message">
    Use the `whatsapp_template` key inside your `flow` array. Set `sender` to your Phone Number ID, `template_name` to the approved template name, and `language` to the template language code. If the template contains variable placeholders (`{{1}}`, `{{2}}`, etc.), pass their values in the `params` array.
...
|-------|----------------------|---------|
| `params` | Message body — `{{1}}`, `{{2}}`, … | `["John", "ORDER-123", "tomorrow"]` |
...
<Note>
  The order of values in each `params` array maps positionally to the `{{1}}`, `{{2}}`, … placeholders in your template. Make sure the array length matches the number of variables in the template or the request will be rejected.
</Note>
