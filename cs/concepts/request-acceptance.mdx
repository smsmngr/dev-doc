...
Zpráva, která touto fází projde, je předána k doručení a dál sledujete její stav běžnou [sekvencí doručenek](/guides/webhooks) (`sending` → `sent` → `delivered`).
...
<Note>
  Aby vám tyto informace dorazily, nastavte pole `callback` na požadavku (nebo výchozí callback URL na API klíči). Bez nastavené callback URL se o zamítnutí zprávy ve fázi 2 nedozvíte. Podrobnosti najdete v návodu [Webhooky](/guides/webhooks).
</Note>
