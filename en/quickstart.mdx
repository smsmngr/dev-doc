...
<Tip>
  **Building the integration with an AI agent?** Install the [SmsManager skills](/ai/skills) (`npx skills add smsmngr/smsmanager-skills`) and the agent handles the steps below on its own — including the phone number format, the `x-api-key` header and response handling. To send a message straight from a conversation without writing code, connect the [MCP server](/ai/mcp-server).
</Tip>
...
<CardGroup cols={2}>
  ...
  <Card title="AI skills and MCP server" icon="wand-magic-sparkles" href="/ai/skills">
    Let an AI agent write the integration for you, or connect an assistant directly to your account.
  </Card>
</CardGroup>