...
<Steps>
  ...
  <Step title="Choose a gateway">
    ...
    | Gateway | Description |
    | --- | --- |
    | `high` | Default. Best delivery rate via the aggregator network. |
    | `lowcost` | Low-cost routing. |
    | `direct` | Required when using a dedicated virtual number as the sender. Routes through direct carrier connections. |
    | `custom` | SIM-hosting setup where you supply your own SIM hardware. |
    | `simhost` | SIM-hosting setup (alternative to `custom`). |
    | `gsm` | Route through your own GSM gateway. |

    You normally don't need to set `gateway` — SmsManager picks it automatically based on the sender.
...
<CardGroup cols={2}>
  <Card title="Batch Sending" icon="layer-group" href="/guides/batch-sending">
    Send up to 100 messages in a single /messages call, or upload a JSONL file with thousands of messages.
  </Card>
