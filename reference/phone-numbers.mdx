...
<Warning>
  Invalid phone numbers cause messages to be **rejected before they are sent**. A rejected message still consumes a validation attempt and may trigger a `result: rejected` webhook callback. Always validate and normalise phone numbers on your side before calling the API, and check the `rejected` array in API responses to identify any numbers that failed.
</Warning>
